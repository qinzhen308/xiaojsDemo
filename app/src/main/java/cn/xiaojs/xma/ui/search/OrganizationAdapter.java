...
public class OrganizationAdapter extends SearchAdapter<AccountInfo, OrganizationAdapter.Holder> {

    private CircleTransform circleTransform;

    public OrganizationAdapter(Context context, PullToRefreshSwipeListView listView) {
    ...
    protected View createContentView(int position) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.layout_search_organization_item, null);
        return view;
    }
    ...
    protected void doRequest() {
        ...
        SearchManager.searchAccounts(mContext, keyWord, mPagination,
                Account.TypeName.ORGANIZATION, new APIServiceCallback<CollectionPageData<AccountInfo>>() {
                    @Override
                    public void onSuccess(CollectionPageData<AccountInfo> object) {
                        if (object != null && object.accounts != null) {
                            OrganizationAdapter.this.onSuccess(object.accounts);
                        } else {
                            OrganizationAdapter.this.onSuccess(null);
                        }
                    }

                    @Override
                    public void onFailure(String errorCode, String errorMessage) {
                        OrganizationAdapter.this.onFailure(errorCode, errorMessage);
                    }
                });
    }
